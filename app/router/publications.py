...
@router.get(
    "/publications/{publication_id}", response_model=schemas.PublicationResponse
)
def get_publication(publication_id: int = Path(), db: SessionLocal = Depends(get_db)):
    ...
    if not publication:
        return HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Publication does not exist"
        )
    return publication
...
def create_publication(
    ...
    return crud.create_publication(
        publication=publication, db=db, user_id=1
    )  # Добавить получение id юзера
...
def change_publication(
    ...
    return HTTPException(
        ...
        detail="You cannot modify this publication",
    )


@router.delete("/publications/{publication_id}")
def delete_publication(
    user_id: int, publication_id: int = Path(), db: SessionLocal = Depends(get_db)
):
    db_publication = crud.get_publication(db=db, publication_id=publication_id)
